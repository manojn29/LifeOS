...
export class GroqProvider implements AIProvider {
  ...
  constructor(apiKey?: string) {
    ...
    if (this.apiKey) {
      try {
        ...
        console.warn('Failed to initialize Groq OpenAI client:', err);
      }
    }
  }
  ...
  async cleanJournal(rawText: string): Promise<string> {
    ...
    try {
      ...
      console.warn('Groq cleanJournal fallback:', err);
      return this.mockCleanJournal(rawText);
    }
  }

  async generateEmbedding(text: string): Promise<number[]> {
    // Groq focus is ultra-fast LLM inference; generate deterministic vector for pgvector
  ...
  async chatWithContext(params: {
    ...
    try {
      ...
      console.error('Groq chat error, falling back:', err);
      return this.mockChatResponse(latestUserMessage, journalContext, mode, toolActions);
    }
  }
  ...
  async generateWeeklyDigest(params: {
    ...
    try {
      ...
      console.warn('Groq weekly digest fallback:', err);
      return {
